...
class DormitoryCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_number(self, number):
        ...
        if flag is None:
            raise serializers.ValidationError('编号须为5~7位的数字和大写英文！')
        if len(Dormitory.objects.filter(number=number)) != 0:
            raise serializers.ValidationError('已存在编号相同的宿舍！')
        return number
    ...
    def validate_area(self, area):
        ...
        if flag is None:
            raise serializers.ValidationError('区域须以<学>字开头，中文数字结尾！')
        return area
    ...
    def validate_build(self, build):
        ...
        if flag is None:
            raise serializers.ValidationError('宿舍楼须为1位大写英文！')
        return build
    ...
    def validate_room(self, room):
        ...
        if flag is None:
            raise serializers.ValidationError('房间号须为3~4位数字！')
        if len(room) == 3 and self.initial_data["floor"] != room[0]:
            raise serializers.ValidationError('房间号前一位与楼层不对应！')
        if len(room) == 4 and self.initial_data["floor"] != room[0:2]:
            raise serializers.ValidationError('房间号前两位与楼层不对应！')
        return room
...
class DormitoryChangeAllowLiveNumberSerializer(serializers.ModelSerializer):
    ...
    def validate_allow_live_number(self, allow_live_number):
        if len(self.instance.lived_users.all()) > allow_live_number:
            raise serializers.ValidationError('宿舍允许居住人数不允许小于现已住人数！')
...
class ElectricityFeesRechargeAdminSerializer(serializers.ModelSerializer):
    """
    宿舍电费 充值 序列类
    """
    money = serializers.DecimalField(max_digits=5, decimal_places=2)
